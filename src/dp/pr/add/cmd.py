...
@app.command()
def urls(
    target_branch_name: str = typer.Option(
        "update/template",
        "--target-branch-name",
        "-tbn",
    ...
    print(
        f"[green]✅ state file {state_name} created and uploaded to {STATE_BUCKET_NAME_URI}"
    )
...
def create_state(
    repo_list: list[str], state_name: str, target_branch_name: str
) -> State:
