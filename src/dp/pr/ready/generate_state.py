...
def generate(state_name: str | None = None, folder_path: Path | None = None) -> None:
    ...
    if folder_path is None:
        print("[yellow]No folder specified, fetching all team repositories from 'terraform-ssb-dapla-teams'")
        print(
            "[yellow]This can take a minute"
        )
            ...
            if team_item.type == "dir":  # Only process directories
                ...
                try:
                    ...
                    if isinstance(team_yaml, list):
                        raise ValueError(f"Team {team_name} has multiple team.yaml files")
                    
                    yaml_content = yaml.safe_load(team_yaml.decoded_content)
                    iac_repo_name = yaml_content.get("github", {}).get("iac_repo", {})["name"]
                    if iac_repo_name:
...
def _get_template_prs_for_repos(team_repos: list[str]) -> dict[str, PullRequest]:
    ...
    for team_repo in team_repos:
        pr = get_last_open_pr_for_user_and_title(repo_name=team_repo, user=GITHUB_TEMPLATE_PR_USER, title=GITHUB_TEMPLATE_PR_TITLE)
        if pr is None:
            print(
                RichWarning(
